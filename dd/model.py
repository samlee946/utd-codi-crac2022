...
class CorefModel(nn.Module):
    ...
    def get_predictions_and_loss(self, input_ids, input_mask, speaker_ids, sentence_len, genre, sentence_map,
        ...
        if len(selected_idx) == 0:
            ...
            return candidate_starts, candidate_ends, candidate_mention_scores, candidate_starts, candidate_ends, np.array([]), np.array([]), None, None
        ...
        if is_training != 0:
            if conf['inference_filter_non_gold_anaphors']:
                anaphor_mask = (top_span_types == entity_type_id_anaphor)
                top_anaphor_pred_types = candidate_mention_type_ids[selected_idx][anaphor_mask].squeeze(1)
                top_anaphor_pred_types = torch.ones(top_anaphor_pred_types.shape, device=device,
                                                    dtype=top_anaphor_pred_types.dtype)
                constraint_anaphor_mask = torch.zeros(top_span_types.shape, dtype=torch.bool,
                                                      device=self.device)
                constraint_anaphor_mask[anaphor_mask] = (top_anaphor_pred_types == entity_type_id_anaphor)
                constraint_utterance_mask = (top_span_types == entity_type_id_antecedent)
            ...
            if conf['inference_filter_antecedent_raw'] > 0:
                antecedent_mask &= (antecedent_sentence_distance < conf['inference_filter_antecedent_raw'])

            if conf['inference_filter_antecedent_new'] > 0:
                antecedent_mask &= (
                        sentence_distance_ignore_empty[selected_idx] < conf['inference_filter_antecedent_new'])

            if conf['inference_filter_num_of_words_equals_zero']:

                anaphor_mask_cpu = anaphor_mask.to('cpu')
                antecedent_mask_cpu = antecedent_mask.to('cpu')
                num_of_words_remove_nonsense_cpu = num_of_words_remove_nonsense.to('cpu')
                top_span_starts_cpu = top_span_starts.to('cpu')
                top_span_ends_cpu = top_span_ends.to('cpu')
                for i in range(top_span_starts.shape[0]):
                    if anaphor_mask_cpu[i]:
                        for j in range(i):
                            if antecedent_mask_cpu[i, j]:
                                utt_st = max(0, top_span_starts_cpu[j] - 1)
                                utt_ed = torch.min(top_span_ends_cpu[j], top_span_starts_cpu[i] - 1)
                                num = num_of_words_remove_nonsense_cpu[utt_ed] - num_of_words_remove_nonsense_cpu[
                                    utt_st]
                                if num == 0:
                                    antecedent_mask_cpu[i, j] = 0
                                    # print(f'{i} {j}')

                antecedent_mask = antecedent_mask_cpu.to(device)
