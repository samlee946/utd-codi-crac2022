...
class Tensorizer:
    ...
    def tensorize_example(self, example, training_flag):
        ...
        if self.config['use_utterace_span_from_file'] == 'Both':
            ...
            if self.config['use_4_dep_features_for_type']:
                assert len(utterance_spans) == len(example['dep_label'])
                utterance_spans, example['dep_label'], example['dep_other_word_pos_tag'], example[
                    'dep_other_word_has_relcl'], example['dep_other_word_spans'] = zip(*sorted(
                    zip(utterance_spans, example['dep_label'], example['dep_other_word_pos_tag'],
                        example['dep_other_word_has_relcl'], example['dep_other_word_spans']), key=lambda x: (x[0][0], -x[0][1])))

                candidate_dep_label_list = []
                candidate_dep_other_word_pos_tag_list = []
                candidate_dep_other_word_has_relcl_list = []
                candidate_dep_other_word_spans_starts_list = []
                candidate_dep_other_word_spans_ends_list = []
            else:
            ...
            for idx, (st, ed, ty) in enumerate(utterance_spans):
                ...
                if self.config['use_4_dep_features_for_type']:
                    # dep parsing 20220226
                    other_word_st = example['dep_other_word_spans'][idx][0]
                    other_word_ed = example['dep_other_word_spans'][idx][1]
                    if other_word_st == -1 or other_word_st > num_words:
                        other_word_st = -1
                        other_word_ed = -1
                        dep_label_value = -1
                        pos_tag_value = -1
                        hasrelcl_value = False
                    else:
                        other_word_st -= 0 # original word_offset, because we did it after the truncation
                        other_word_ed -= 0
                        other_word_ed = min(other_word_ed, num_words)
                        dep_label = example['dep_label'][idx]
                        pos_tag = example['dep_other_word_pos_tag'][idx]
                        if dep_label != 'null':
                            dep_label_value = self.stored_info['dep_tag_dict'][dep_label]
                        if pos_tag != 'null':
                            pos_tag_value = self.stored_info['pos_tag_dict'][pos_tag]
                        hasrelcl_value = example['dep_other_word_has_relcl'][idx]
                    candidate_dep_other_word_spans_starts_list.append(other_word_st)
                    candidate_dep_other_word_spans_ends_list.append(other_word_ed)
                    candidate_dep_label_list.append(dep_label_value)
                    candidate_dep_other_word_pos_tag_list.append(pos_tag_value)
                    candidate_dep_other_word_has_relcl_list.append(hasrelcl_value)
            ...
            if self.config['use_4_dep_features_for_type']:
                candidate_dep_label = np.array(candidate_dep_label_list, dtype=np.int)
                candidate_dep_other_word_pos_tag = np.array(candidate_dep_other_word_pos_tag_list, dtype=np.int)
                candidate_dep_other_word_has_relcl = np.array(candidate_dep_other_word_has_relcl_list, dtype=np.int)
                candidate_dep_other_word_spans_starts = np.array(candidate_dep_other_word_spans_starts_list, dtype=np.int)
                candidate_dep_other_word_spans_ends = np.array(candidate_dep_other_word_spans_ends_list, dtype=np.int)


        elif self.config['use_utterace_span_from_file'] == 'OnlyAntecedent':

            assert not self.config['use_4_dep_features_for_type']
