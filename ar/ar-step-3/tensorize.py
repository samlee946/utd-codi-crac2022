...
class CorefDataProcessor:
    ...
    def get_cache_path(self):
        ...
        cache_path = join(self.data_dir,
                          f'cached.AnaPredThird.{self.train_dir}.{self.dev_dir}.ErrMen{self.config["error_men_in_second_step"]}.{bool(self.config["is_pretraining"])}.utt{bool(self.config["pronoun_prev_utt"]>0)}.{self.config["max_span_width"]}.{self.config["num_entity_types"]}.{self.max_seg_len}.{self.max_training_seg}.bin')  # MODIFIED
        return cache_path
