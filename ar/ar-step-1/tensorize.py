...
class CorefDataProcessor:
    ...
    def get_cache_path(self):
        ...
        cache_path = join(self.data_dir,
                          f'cached.tensors.{self.train_dir}.{self.dev_dir}.{bool(self.config["is_pretraining"])}.utt{bool(self.config["pronoun_prev_utt"]>0)}.{self.config["max_span_width"]}.{self.max_seg_len}.{self.max_training_seg}.bin')  # MODIFIED
        return cache_path
    ...
    def tensorize_example(self, example, training_flag):
        ...
        for example_tensor in example_tensors:
            ...
            if not self.config['is_second_step']:
                ...
                if self.config['error_men_in_second_step']:
