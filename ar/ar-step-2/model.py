...
class CorefModel(nn.Module):
    def __init__(self, config, device, num_genres=None):
        ...
        self.span_emb_score_ffnn = self.make_ffnn(self.span_emb_size, [config['ffnn_size']] * config['ffnn_depth'], output_size=1) if config['use_mention_scores'] else None
        ...
        if config['use_type_prediction_model']:
        ...
        # anaphoric
        self.tot_anaphoric = 0
        self.case1 = 0 # correctly resolved an non-null antecedent
        self.case2 = 0 # gold antecedent not top 50 but resolved to null
        self.case3 = 0 # gold antecedent not top 50 but resolved to non-null
        self.case4 = 0 # gold antecedent among 50 but resolved to null
        self.case5 = 0 # gold antecedent among 50 but resolved to non-null not-gold antecedent
        # non-anaphoric
        self.tot_nonanaphoric = 0
        self.case11 = 0 # correctly resolved to null
        self.case12 = 0 # incorrectly resolved to non-null
    ...
    def print_stats(self):
        print('{:80s} {:<8d}'.format('#total:', self.tot_men))
        print('{:80s} {:<8d} {:<.2f}%'.format('#anaphoric:', self.tot_anaphoric, self.tot_anaphoric / self.tot_men * 100.0))
        print('{:80s} {:<8d} {:<.2f}%'.format('#correctly resolved an non-null antecedent:', self.case1, self.case1 / self.tot_anaphoric * 100.0))
        print('{:80s} {:<8d} {:<.2f}%'.format('#gold antecedent not top 50 but resolved to null:', self.case2, self.case2 / self.tot_anaphoric * 100.0))
        print('{:80s} {:<8d} {:<.2f}%'.format('#gold antecedent not top 50 but resolved to non-null:', self.case3, self.case3 / self.tot_anaphoric * 100.0))
        print('{:80s} {:<8d} {:<.2f}%'.format('#gold antecedent among 50 but resolved to null:', self.case4, self.case4 / self.tot_anaphoric * 100.0))
        print('{:80s} {:<8d} {:<.2f}%'.format('#gold antecedent among 50 but resolved to non-null not-gold antecedent:', self.case5, self.case5 / self.tot_anaphoric * 100.0))
        print('{:80s} {:<8d} {:<.2f}%'.format('#non-anaphoric:', self.tot_nonanaphoric, self.tot_nonanaphoric / self.tot_men * 100.0))
        print('{:80s} {:<8d} {:<.2f}%'.format('#correctly resolved to null:', self.case11, self.case11 / self.tot_nonanaphoric * 100.0))
        print('{:80s} {:<8d} {:<.2f}%'.format('#incorrectly resolved to non-null:', self.case12, self.case12 / self.tot_nonanaphoric * 100.0))
    ...
    def get_predictions_and_loss(self, input_ids, input_mask, speaker_ids, sentence_len, genre, sentence_map, is_pronoun,
        ...
        if conf['is_second_step']:
            ...
            assert conf['use_mention_scores']
        ...
        #
        # if 'sent_dist_filtering' in conf:
        #     antecedent_mask &= antecedent_sentence_distance <= conf['sent_dist_filtering']
        ...
        # Get loss
        if 'null_score' in conf:
            top_antecedent_scores = torch.cat([torch.ones(num_top_spans, 1, device=device)*float(conf['null_score']), top_pairwise_scores],
                                              dim=1)
        else:
        ...
        if is_training == 2:
            # TODO: do stats
            # anaphoric
            stat_top_span_cluster_ids_cpu = top_span_cluster_ids.to('cpu')
            stat_anaphoric_mask_cpu = (stat_top_span_cluster_ids_cpu > 0).tolist()
            stat_top_antecedent_gold_labels_cpu = top_antecedent_gold_labels.to('cpu').tolist()
            stat_idx = torch.nonzero(top_span_cluster_ids > 0).squeeze(1)
            stat_visited = {}
            for idx in stat_idx:
                stat_cluster_id_temp = stat_top_span_cluster_ids_cpu[idx].item()
                if stat_cluster_id_temp not in stat_visited:
                    stat_anaphoric_mask_cpu[idx] = False
                stat_visited[stat_cluster_id_temp] = True
            stat_antecedent_scores = top_antecedent_scores.tolist()
            for i, ant_idx in enumerate(np.argmax(stat_antecedent_scores, axis=1)):
                self.tot_men += 1
                if stat_anaphoric_mask_cpu[i]: self.tot_anaphoric += 1
                else: self.tot_nonanaphoric += 1
                if ant_idx == 0:
                    if not stat_anaphoric_mask_cpu[i]:
                        self.case11 += 1  # non-anaphoric: correctly resolved to null
                    elif stat_top_antecedent_gold_labels_cpu[i][0]:
                        self.case2 += 1   # anaphoric: gold antecedent not top 50 but resolved to null
                    else:
                        # anaphoric: gold antecedent among 50 but resolved to null
                        self.case4 += 1
                else:
                    if not stat_anaphoric_mask_cpu[i]:
                        self.case12 += 1
                    else:
                        if stat_top_antecedent_gold_labels_cpu[i][ant_idx]:
                            self.case1 += 1 # anaphoric: correctly resolved an non-null antecedent
                        elif stat_top_antecedent_gold_labels_cpu[i][0]:
                            self.case3 += 1 # anaphoric: gold antecedent not top 50 but resolved to non-null
                        else:
                            self.case5 += 1 # anaphoric: gold antecedent among 50 but resolved to non-null

        # Add mention loss
        if conf['use_mention_scores']:
            assert conf['mention_loss_coef']
    ...
    def get_predicted_clusters(self, span_starts, span_ends, span_speaker, antecedent_idx, antecedent_scores, top_span_pred_types=None, top_span_gold_types=None):
        ...
        # Add singletons
        if self.config['use_type_for_singletons']:
            assert top_span_pred_types is not None
            for i, predicted_idx in enumerate(span_starts):
                ...
                #     # else:
                #     continue
                if top_span_pred_types[i] < self.config['num_entity_types'] - 1:
                    continue
                mention = (int(span_starts[i]), int(span_ends[i]))
