...
if __name__ == '__main__':
    ...
    parser.add_argument('--evaluate_gold', type=str, default=None)
    args = parser.parse_args()

    runner = Runner(args.config_name, args.gpu_id, args.seed)
    model_identifier = args.model_identifier.replace('model_', '').replace('.bin', '')
    ...
    if args.jsonlines_path:
        ...
        #
        #     if args.output_path:
        #         with open(args.output_path, 'w' if idx == 0 else 'a') as f:
        #             doc['predicted_clusters'] = predicted_clusters[0]
        ...
        if args.output_path:
            with open(args.output_path, 'w') as f:
                for idx, doc in enumerate(docs):
                    doc['predicted_clusters'] = predicted_clusters[idx]
                    f.write(json.dumps(doc))
                    f.write('\n') # MODIFIED
            print(f'Saved prediction in {args.output_path}')
        ...
        if args.evaluate_gold is not None:
            import subprocess
            subprocess.call(['python', 'ua-scorer.py', args.evaluate_gold, conll_path])
