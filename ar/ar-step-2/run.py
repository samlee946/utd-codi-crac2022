...
class Evaluator:
    ...
    def conll_score(self):
        ...
        model_to_evaluate.init_stats()

        data_processor = self.runner.data
        jsonlines_path = join(self.runner.config['data_dir'], self.runner.config['test_dir'])
        output_path = join(self.runner.config['data_dir'], 'output_{}_{}_{}_ns4.jsonlines'.format(self.runner.name, self.runner.best_model_suffix, self.runner.config['test_dir'].rstrip('.jsonlines')))
        ...
        with open(output_path, 'w') as f:
            for i, doc in enumerate(docs):
                docs[i]['first_step_predicted_clusters'] = docs[i]['predicted_clusters']
                docs[i]['predicted_clusters'] = predicted_clusters[i]
                # doc['clusters'] = predicted_clusters[i]  # MODIFIED
                f.write(json.dumps(docs[i]))
                f.write('\n')  # MODIFIED
        print(f'Saved prediction in {output_path}')
        ...
        if not self.runner.config['prediction_only']:
            ...
            for doc in docs:
                ...
                all_mentions_sys = set([(men_st, men_ed) for cluster in doc['predicted_clusters'] for (men_st, men_ed) in cluster])
                all_mentions_first_step_prediction = set([(men_st, men_ed) for cluster in doc['first_step_predicted_clusters'] for (men_st, men_ed) in cluster])
                mde.update(None, None, mention_to_sys=mention_to_sys, mention_to_gold=mention_to_gold)
                # print(len(all_mentions_sys), sorted(all_mentions_sys))
                # print(len(all_mentions_first_step_prediction), sorted(all_mentions_first_step_prediction))
                # print(all_mentions_sys == all_mentions_first_step_prediction)
                # import ipdb
                # ipdb.set_trace()
...
def batch_evaluate(gpu_id):
    test_files = [
        (
        'output_codi2022_arpred_first_step_subm_tl500_30_nocons_Jun28_16-06-56_10.0_light_test_no_men.2022.ar.pron10.jsonlines.jsonlines',
        None),
        (
        'output_codi2022_arpred_first_step_subm_tl500_30_nocons_Jun28_16-06-56_20.0_AMI_test_no_men.2022.split.ar.pron10.split.jsonlines.jsonlines',
        None),
        (
        'output_codi2022_arpred_first_step_subm_tl500_30_nocons_Jun28_16-06-56_20.0_Persuasion_test_no_men.2022.ar.pron10.jsonlines.jsonlines',
        None),
        (
        'output_codi2022_arpred_first_step_subm_tl500_30_nocons_Jun28_16-06-56_10.0_Switchboard_test_Phase_1.2022.ar.pron10.jsonlines.jsonlines',
        None),
    ]

    configs = [
        # subm
        ('codi2022_arpred_second_step_subm_tl0_NoPronounUtt_GoldMen_30_noMenScores_pronCon',
         'model_Jul06_20-25-42_10.0.bin', 11),
        ('codi2022_arpred_second_step_subm_tl0_NoPronounUtt_GoldMen_30_noMenScores_pronCon',
         'model_Jul06_20-25-42_15.0.bin', 11),
        ('codi2022_arpred_second_step_subm_tl0_NoPronounUtt_GoldMen_30_noMenScores_pronCon',
         'model_Jul06_20-25-42_20.0.bin', 11),
        ('codi2022_arpred_second_step_subm_tl0_NoPronounUtt_GoldMen_30_noMenScores_pronCon',
         'model_Jul06_20-25-42_25.0.bin', 11),
        ('codi2022_arpred_second_step_subm_tl0_NoPronounUtt_GoldMen_30_noMenScores_pronCon',
         'model_Jul06_20-25-42_5.0.bin', 11),
    ]
    for config_name, model_to_evaluate, seed_to_eval in configs:
        ...
        for test_fn, gold_conll in test_files:
            ...
            runner.config['null_score'] = 4
            # runner.config['pronoun_prev_utt'] = sd
