...
Jsonlines key-value format:
    ...
    'pronouns': []
'''
def convert_coref_ua_to_json_coref_hoi(UA_PATH, JSON_PATH, SEGMENT_SIZE, TOKENIZER_NAME, no_men):
    ...
    with open(JSON_PATH, "w") as output_file:
        for doc in key_doc_sents:
            print(doc)
            document_ar = get_document(doc, key_docs[doc], 'english', {}, SEGMENT_SIZE, tokenizer, dd=False, no_men=no_men)
            output_file.write(json.dumps(document_ar))
            output_file.write('\n')
            
def convert_dd_to_json(UA_PATH, JSON_PATH, all_anaphor, SEGMENT_SIZE, TOKENIZER_NAME):
    ...
    with open(JSON_PATH, "w") as output_file:
        for doc in key_doc_sents:
            print(doc)
            document = get_document(doc, key_docs[doc], 'english', all_anaphor, SEGMENT_SIZE, tokenizer, dd=True)
            if len(document['utterance_span']):
                output_file.write(json.dumps(document))
                output_file.write('\n')
            if document['doc_key'] == 'RST_DTreeBank/train/wsjarrau_1384':
                print(len(document['utterance_span']))
            
def convert_to_json_ar_dd(UA_PATH, JSON_PATH, all_coref_anaphor, all_anaphor, SEGMENT_SIZE, TOKENIZER_NAME, no_men, dd_only=False):
    ...
    with open(JSON_PATH, "w") as output_file:
        for doc in key_doc_sents:
            print(doc)
            if not dd_only:
                document_ar = get_document(doc, key_docs[doc], 'english', all_coref_anaphor, SEGMENT_SIZE, tokenizer, dd=False, no_men=no_men)
                document_dd = get_document(doc, key_docs[doc], 'english', all_anaphor, SEGMENT_SIZE, tokenizer, dd=True, no_men=no_men)
                document = document_ar
                anaphor_mentions = set([(m[0], m[1]) for m in document_dd['utterance_span'] if m[2] == 'anaphor'])
                utterance_mentions = [(m[0], m[1]) for m in document_dd['utterance_span'] if m[2] == 'utterance']
#                 print(document_ar['clusters'])
#                 print(anaphor_mentions)
                for c_id, c in enumerate(document_ar['clusters']):
                    for m_id, m in enumerate(c):
                        m_t = (m[0], m[1])
                        if m_t in anaphor_mentions:
                            document_ar['clusters'][c_id][m_id] = (m[0], m[1], 'anaphor')
                document['utterance_span'] = document_dd['utterance_span'] # caution! edited 2021/12/20 for 11/15 model
#                 document['following_5_words'] = document_dd['following_5_words'] # edited 20220323
#                 document['preceding_5_words'] = document_dd['preceding_5_words']
                document['clusters_dd'] = document_dd['clusters'] 
                document['clusters_ar'] = document_ar['clusters']
                document['gold_anaphors'] = document_dd['gold_anaphors'] 
            else:
                document_dd = get_document(doc, key_docs[doc], 'english', all_anaphor, SEGMENT_SIZE, tokenizer, dd=True, no_men=no_men)
                document = document_dd
#                 anaphor_mentions = set([(m[0], m[1]) for m in document_dd['utterance_span'] if m[2] == 'anaphor'])
#                 utterance_mentions = [(m[0], m[1]) for m in document_dd['utterance_span'] if m[2] == 'utterance']
#                 document['utterance_span'] = utterance_mentions
                document['clusters_dd'] = document_dd['clusters']
            del document['clusters']
            output_file.write(json.dumps(document))
            output_file.write('\n')
